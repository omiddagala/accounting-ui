...
export class MyDatePicker implements OnChanges, ControlValueAccessor, OnDestroy, OnInit {
  dateForm: FormGroup;
      ...
      for (let i = 1; i <= 12; i += 3) {
        ...
        for (let j = i; j < i + 3; j++) {
          let disabled: boolean = this.utilService.isMonthDisabledByDisableUntil({
            ...
            }, this.opts.disableUntil, this.type)
            || this.utilService.isMonthDisabledByDisableSince({
              year: this.visibleMonth.year,
              month: j,
              day: 1
            }, this.opts.disableSince, this.type);
          row.push({
            nbr: j,
  ...
  onYearCellClicked(cell: IMyCalendarYear): void {
    ...
    this.selectYear = false;
    console.log(this.selectorEl.nativeElement)
    this.selectorEl.nativeElement.focus();
  }
  ...
  onUserDateInput(value: string): void {
    if (value.length === 0) {
      ...
      let date: IMyDate = this.utilService.isDateValid(value, this.opts.dateFormat, this.opts.minYear, this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableWeekdays, this.opts.disableDays, this.opts.disableDateRanges, this.opts.monthLabels, this.opts.enableDays, this.type);
  ...
  onBlurInput(event: any): void {
    ...
    this.dateForm.get('datepicker').setValue(event.target.value);
    this.onTouchedCb();
  ...
  isTodayDisabled(): void {
    this.disableTodayBtn = this.utilService.isDisabledDay(this.getToday(), this.opts.minYear, this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableWeekdays, this.opts.disableDays, this.opts.disableDateRanges, this.opts.enableDays, this.type);
  }
  ...
  writeValue(value: any): void {
    if (value && (value['date'] || value['jsdate'] || value['formatted'])) {
      ...
      this.dateForm.get('datepicker').setValue(this.utilService.formatDate(this.selectedDate, this.opts.dateFormat, this.opts.monthLabels));
      ...
      this.dateForm.get('datepicker').setValue('')
    }
    this.inputFieldChanged.emit({
      // value: this.selectionDayTxt,
      value: this.dateForm.get('datepicker').value,
      dateFormat: this.opts.dateFormat,
      // valid: this.selectionDayTxt.length > 0
      valid: this.dateForm.get('datepicker').value > 0
    });
  ...
  updateDateValue(date: IMyDate): void {
    ...
    if (!this.opts.inline) {
      this.dateForm.get('datepicker').setValue(clear ? '' : this.utilService.formatDate(date, this.opts.dateFormat, this.opts.monthLabels));
      ...
      this.inputFieldChanged.emit({value: this.dateForm.get('datepicker').value, dateFormat: this.opts.dateFormat, valid: !clear});
      this.invalidDate = false;
    }
  }
  ...
  monthStartIdx(_y: number, _m: number): number {
    // Month start index
    let d = new Date();
    let day: number = 1, y = _y, m = _m;
    if (this.type === 'jalali') {
      day = this.utilService.jalaliToGregorian(_y, _m, 1).d;
      m = this.utilService.jalaliToGregorian(_y, _m, 1).m;
      y = this.utilService.jalaliToGregorian(_y, _m, 1).y;
    }
    d.setDate(day);
    d.setMonth(m - 1);
    d.setFullYear(y);
  ...
  daysInMonth(m: number, y: number): number {
    ...
    return this.utilService.getDaysInMonth(m, y, this.type);
  }

  daysInPrevMonth(_m: number, _y: number): number {
    let day: number = 1, y = _y, m = _m;
    if (this.type === 'jalali') {
      day = this.utilService.jalaliToGregorian(_y, _m, 1).d;
      m = this.utilService.jalaliToGregorian(_y, _m, 1).m;
    ...
    let d: Date = this.getDate(y, m, day);
    d.setMonth(d.getMonth() - 1);
  ...
  getToday(): IMyDate {
    ...
    if (this.type == 'jalali') {
      let JDate = this.utilService.gregorianToJalali(date.getFullYear(), date.getMonth() + 1, date.getDate());
      return {year: JDate.y, month: JDate.m, day: JDate.d};
    }
  ...
  getWeekday(date: IMyDate): string {
    ...
    let weekDays: Array<string> = this.utilService.getWeekDays();
    return weekDays[this.utilService.getDayNumber(date, this.type)];
  }
  ...
  generateCalendar(m: number, y: number, notifyChange: boolean): void {
    ...
    for (let i = 1; i < 7; i++) {
      ...
      if (i === 1) {
        ...
        for (let j = pm; j <= dInPrevM; j++) {
          ...
          week.push({
            ...
            disabled: this.utilService.isDisabledDay(date, this.opts.minYear, this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableWeekdays, this.opts.disableDays, this.opts.disableDateRanges, this.opts.enableDays, this.type),
            markedDate: this.utilService.isMarkedDate(date, this.opts.markDates, this.opts.markWeekends, this.type),
            highlight: this.utilService.isHighlightedDate(date, this.opts.highlightDay, this.opts.highlightDates, this.type)
          });
        }
      ...
      let weekNbr: number = this.opts.showWeekNumbers && this.opts.firstDayOfWeek === 'mo' ? this.utilService.getWeekNumber(week[0].dateObj, this.type) : 0;
  ...
  parseSelectedDate(selDate: any): IMyDate {
    ...
    this.dateForm.get('datepicker').setValue(this.utilService.formatDate(date, this.opts.dateFormat, this.opts.monthLabels));
    return date;
  }
